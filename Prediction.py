...
spark = SparkSession(spark_conf)

#Load the validation dataset from argument
path  = sys.argv[1]
...
def to_labeled_point(spark_conf, features, labels, categorical=False):
    labeled_points = []
...
dataset = to_labeled_point(spark_conf, features, label)
#path2  = sys.argv[2]
RFModel = RandomForestModel.load(spark_conf, "output/trainingmodel.model/")
